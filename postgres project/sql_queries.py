...
songplay_table_create = "CREATE TABLE IF NOT EXISTS songplays (songplay_id int, start_time timestamp, user_id int, level varchar, song_id varchar, artist_id varchar, session_id int, location varchar, user_agent varchar, PRIMARY KEY (songplay_id, start_time, user_id, session_id));"
...
song_table_create = "CREATE TABLE IF NOT EXISTS songs (song_id varchar NOT NULL PRIMARY KEY, title varchar, artist_id varchar, year int, duration numeric);"

artist_table_create = "CREATE TABLE IF NOT EXISTS artists (artist_id varchar NOT NULL PRIMARY KEY, name varchar, location varchar, latitude numeric, longitude numeric);"
...
songplay_table_insert = "INSERT INTO songplays (songplay_id, start_time, user_id, level, song_id, artist_id, session_id, location, user_agent) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (songplay_id, start_time, user_id, session_id) DO UPDATE SET level  = EXCLUDED.level, artist_id  = EXCLUDED.artist_id, location = EXCLUDED.location, user_agent = EXCLUDED.user_agent;"
